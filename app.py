...
st.set_page_config(
    page_title="台指期實時關卡分析圖", page_icon="📊", layout="wide"
)
...
if plot_df.empty:
  st.warning(
      "⏳ 尚未取得行情資料！請確認 XQ 腳本正常運作，或檢查網路連線。"
  )
else:
  # 重組 K 線
  ...
  with col_n:
    ...
    n_input = st.number_input(
        "【關卡基準點 N】", value=int(default_n), step=50
    )
  ...
  fig.update_layout(
      ...
      title={
          "text": (
              f"<b>[{data_source}] 台指期 {tf} 關卡圖 (更新時間:"
              f" {now_str})</b>"
          ),
          "x": 0.5,
          "xanchor": "center",
